...
def load_and_process_data(filepath):
    ...
    for _, row in df.iterrows():
        # Skip processing if teams are not equal in size and the smaller team loses
        if len(row['Heroes_Team']) != len(row['Villains_Team']):
            smaller_team = 'Heroes' if len(row['Heroes_Team']) < len(row['Villains_Team']) else 'Villains'
            if smaller_team != row['Winning_Team']:
                continue  # skip
    ...
    for player in player_stats:
        ...
        if stats['matches'] > 0:  # Ensure division by zero is handled
